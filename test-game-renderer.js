...
runner.describe('用户交互', () => {
  ...
  runner.it('鼠标悬停在卡牌上应该显示预览', () => {
    ...
    const preview = renderer.container.querySelector('.card-preview');
...
runner.describe('视觉反馈', () => {
  ...
  runner.it('应该显示错误反馈（红色）', () => {
    ...
    const originalAppendChild = renderer.container.appendChild;
    const originalRemoveChild = renderer.container.removeChild;

    renderer.container.appendChild = (element) => {
      appendChildCalled = true;
      return originalAppendChild.call(renderer.container, element);
    };

    renderer.container.removeChild = (element) => {
      removeChildCalled = true;
      return originalRemoveChild.call(renderer.container, element);
    };
    ...
    setTimeout(() => {
      ...
      // 恢复原始方法
      renderer.container.appendChild = originalAppendChild;
      renderer.container.removeChild = originalRemoveChild;
    }, 2100);
  });
...
runner.describe('性能优化', () => {
  ...
  runner.it('卡牌预览应该跟随鼠标', () => {
    ...
    testPositions.forEach((pos, index) => {
      // 清理之前的预览
      const existingPreview = renderer.container.querySelector('.card-preview');
